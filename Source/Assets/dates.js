(() => {
  ...
  Dates.prototype = {
    ...
    daysOfWeek: ["Hé", "Ke", "Sze", "Csü", "Pé", "Szo", "Va"],
    ...
    showCalendar() {
      ...
      this.calendarDOM.classList = `dates--calendar ${this.theme}`;
      this.calendarDOM.innerHTML = `
      <div class="dates--controls">
        <button class="dates--month-decrement" ${
        ...
        <div class="dates--control-selects">
        <select class="dates--month-select">
          ${this.months.map(
            (month, i) =>
              `<option ${
        ...
        <select class="dates--year-select">
        ...
        <button class="dates--month-increment" ${
          this.canNavigateNext() ? "" : "disabled"
            ...
            ${new Array(7)
              ...
              .map(
                (_, i) =>
                  `<p class="dates--day-of-week">${this.daysOfWeek[i]}</p>`
              )
              .join("")}
            </div>
      ...
      this.calendarDOM
        .querySelector(".dates--year-select")
        .addEventListener("change", () => {
          this.currentDate.setFullYear(
            this.calendarDOM.querySelector(".dates--year-select").value
          );
          this.showCalendar();
        });
      this.calendarDOM
        .querySelector(".dates--month-select")
        .addEventListener("change", () => {
          this.currentDate.setMonth(
            this.calendarDOM.querySelector(".dates--month-select").value
          );
          this.showCalendar();
        });
      this.calendarDOM
        .querySelector(".dates--month-decrement")
      ...
      this.calendarDOM
        .querySelector(".dates--month-increment")
        .addEventListener("click", (e) => {
          e.stopPropagation();
          this.changeMonth(1);
          this.showCalendar();
    ...
    renderCalendar() {
      this.calendarDOM.querySelector(".dates--grid.days").innerHTML = `
        ${this.getCalendarDays()
          .map((d) => {
            return `<button ${
              ...
              this.selectionType == "range"
                ...
                : ""
            } " dates-date="${d.toDateString()}"><div>${d.getDate()}</div></button>`;
          })
          .join("")}
      `;

      this.calendarDOM
        .querySelector(".dates--grid.days")
        .querySelectorAll(".dates--day")
        .forEach((btn) =>
          btn.addEventListener("click", (e) => {
            e.stopPropagation();
            this.onDatePicked(btn.getAttribute("dates-date"));
          })
        );
      this.displayValue();
    },
